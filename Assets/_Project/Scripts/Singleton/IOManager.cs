...
public class IOManager : GenericSingleton<IOManager>
{
    ...
    private string _saveLeaderboardFile;
    ...
    private void Start()
    {
        ...
        _saveLeaderboardFile = Application.persistentDataPath + "/LeaderboardData.json";
    }
    ...
    public void AddHighscoreEntry(int Time, string Name)
    {
        ...
        if (!File.Exists(_saveLeaderboardFile))
        {
            ...
            Debug.Log("Leaderboard creata e salvata in: " + _saveLeaderboardFile);
        }
        else
        {
            string json = File.ReadAllText(_saveLeaderboardFile);
